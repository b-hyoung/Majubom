...
테이블:
  beds          — 침대(환자) 등록
  (2026-09-10: WiFi CSI 관련 테이블·로직은 기능 자체를 완전히 제거하기로 해 삭제됨 —
   csi_readings/baselines 테이블, insert_csi/get_csi_*/compute_alert/update_baseline 함수 전부.
   ToF/mmWave baseline 로직은 tof_logic.py/mmw_logic.py가 각자의 JSON 파일로 독립적으로
   담당하므로 이 삭제로 영향받지 않는다.)
"""
