...
```
> ⚠️ **신호강도(autocorr_strength)는 total_abs에서 제외.** 생체 신호가 아니라 측정 품질 지표 →
> 이상탐지 점수에 넣으면 환경 노이즈가 가짜 경고 유발. **신뢰 게이트(reliable, 강도≥0.30)로만 사용.**
> (z_strength는 참고용으로 계산·표시는 가능하나 total_abs엔 합산하지 않음)
